...
export default function QuatroBall({
	windowWidth,
	windowHeight,
	topLeftPaddleY,
	bottomLeftPaddleY,
	...
	const handlePaddleCollision = useCallback(() => {
		if (
			...
			y + ballContext.size - 5 >= topRightPaddleY &&
			y + 5 <= topRightPaddleY + paddleContext.height &&
			!xBounced
		) {
			setXSpeed((prev) => -prev)
			setXBounced(true)
		...
		if (
			x + ballContext.size + 5 >=
				windowWidth - paddleContext.width - paddleContext.x &&
			y + ballContext.size - 5 >= bottomRightPaddleY &&
			y + 5 <= bottomRightPaddleY + paddleContext.height &&
			!xBounced
		) {
			setXSpeed((prev) => -prev)
			setXBounced(true)
		...
		if (
			...
			y + ballContext.size >= topLeftPaddleY &&
			y <= topLeftPaddleY + paddleContext.height &&
			!xBounced
		) {
			setXSpeed((prev) => -prev)
			setXBounced(true)
		...
		if (
			x - 5 <= paddleContext.width + paddleContext.x &&
			y + ballContext.size >= bottomLeftPaddleY &&
			y <= bottomLeftPaddleY + paddleContext.height &&
			!xBounced
		) {
			setXSpeed((prev) => -prev)
			setXBounced(true)
